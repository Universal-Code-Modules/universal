...
const {
    ...
    // language.classification({text, model}) => responce
    files,
    ...
    describe('Openai Chat', () => {
        ...
        test('Chat voice message', async () => {
            const res = await chat.voiceMessage({
                inputFilePath:'./lib/LLM/OpenAI/tests/speech/test-speech-input-en.mp3', 
                outputFilePath:'./lib/LLM/OpenAI/tests/speech/test-speech-output-en.mp3',
                returnIntermediateResult:false});
        
            expect(res).toHaveProperty('inputText');
            expect(res.inputText).toBe('Hello there.');
            expect(res).toHaveProperty('outputText');
            expect(res).toHaveProperty('outputFilePath');
        });

    })

    describe('Openai Language', () => {
        ...
        test('Text completeon with function call', async () => {
            ...
            const res = await language.generate({text: "What's the weather like in San Francisco, Tokyo, and Paris?", tools:testLibrary.tools});
        ...
        test('Text Embedding', async () => {
            ...
            res = await language.embedding({text:'hello'});
        ...
        test('Text Classification', async () => {
            const res = await language.classification({text:'I will kill you boatman'});
