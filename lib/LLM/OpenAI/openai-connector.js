...
class Chat{
  ...
  constructor({system, model = "gpt-4-1106-preview", tools, maxTokens = 1000}){
    this.system = system;
    this.model = model;
    this.messages = [];
    this.tokens = 0;
...
const defineTools = (params = []) => {
  ...
  const nlp = {
    ...
    embedding: async({text = "Sample text", model = 'text-embedding-3-small'}) => {
      ...
      return {embedding:responce.data[0].embedding, usage:price.calculate({purpose:'embedding', model, usage:responce.usage})};
    },
